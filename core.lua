...
local function TablePrint(table, depth)
    depth = depth or 0
    local indent = ""
    for i = 1, depth do
        indent = indent..".. "
...
function sjUF:OnInitialize()
    ...
    -- Create frames
    self.frames = {}
    self.frames.Master = CreateFrame("Frame", "sjUF_Master", UIParent)
    --self.frames.Player = CreateFrame("Button", "sjUF_Player", self.frames.Master)
    --self.frames.Pet    = CreateFrame("Button", "sjUF_Pet", self.frames.Master)
    --self.frames.Target = CreateFrame("Button", "sjUF_Target", self.frames.Master)
    --self.frames.ToT    = CreateFrame("Button", "sjUF_ToT", self.frames.Master)
    --self.frames.ToToT  = CreateFrame("Button", "sjUF_ToToT", self.frames.Master)
    self.frames.Raid   = CreateFrame("Frame", "sjUF_Raid", self.frames.Master)
    for i = 1, 40 do
        self.frames["Raid"..i] = CreateFrame("Button", "sjUF_Raid"..i, self.frames.Raid)
    end

    -- Local frame references
    master = self.frames.Master
    --player = self.frames.Player
    --pet    = self.frames.Pet
    --target = self.frames.Target
    --tot    = self.frames.ToT
    --totot  = self.frames.ToToT
    raid   = self.frames.Raid
end

--- Ace OnEnable handler.
function sjUF:OnEnable()
    self:Debug(e.."OnEnable")
    ...
    -- Update frame styles always
    -- Update unit data always

    raid:SetWidth(40*8)
    raid:SetHeight(30*5)
    raid:ClearAllPoints()
    raid:SetPoint("TOPLEFT", nil)
    raid.background = raid:CreateTexture(nil, "BACKGROUND")
    raid.background:SetAllPoints(raid)
    raid.background:SetTexture(0, 0, 0, 0.3)
    MakeFrameMovable(raid)
    raid:EnableMouse(true)
...
--- Ace OnDisable handler.
function sjUF:OnDisable()
    self:Debug(e.."OnDisable")
    --self.master:Hide()
end

--- RosterLib_RosterChanged event handler.
function sjUF:OnRosterChanged(table)
    self:Debug(e.."OnRosterChanged")
...
function sjUF:OnPlayerEnteringWorld()
    self:Debug(e.."OnPlayerEnteringWorld")
    if self.group_status ~= self:CheckGroupStatus() then
        self:UpdateFrames()
    end
end
